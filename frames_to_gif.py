...
frames_to_gif(read_input_imgs, 'unaligned.gif')
imave = np.average(np.array([np.array(im) for im in read_input_imgs]),axis=0)
cv2.imwrite('unaligned_mean.png', imave)

alignImages(read_input_imgs)

frames_to_gif(read_input_imgs, 'aligned.gif')
imave = np.average(np.array([np.array(im) for im in read_input_imgs]),axis=0)
cv2.imwrite('aligned_mean.png', imave)
